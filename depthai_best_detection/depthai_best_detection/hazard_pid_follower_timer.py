...
class HazardFollower(Node):
    def __init__(self):
        ...
        # Time out condiiton
        self.pid_runtime_sec = TIMEOUT_SECS
        ...
        self.default_drive_timer = self.create_timer(0.1, self.default_drive)
   ...
   #     self.twist_pub.publish(self.twist)
    ...
    def det_callback(self, msg: BestDetection):
       ...
       # "Oxidizing_substances 0.55 0.25 0.30 0.60 0.80"
        ...
        if self.pid_start_time is not None:
            elapsed = time.time() - self.pid_start_time
            if elapsed >= self.pid_runtime_sec:
                self.get_logger().info("PID timeout reached — stopping robot.")
